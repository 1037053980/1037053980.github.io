...
public class UserController extends BaseController{ //ProductController,CartController
    ...
    @GetMapping("/is_administer")
    public R<Integer> isAdminister(HttpSession session){
        Integer id = getUidFromSession(session);
        Integer data = userService.isAdminister(id);
        return new R<>(OK,data);
    }

    @GetMapping("/detail")
    public R<UserDetail> getUserDetail(HttpSession session){
        Integer id = getUidFromSession(session);
        UserDetail data = userDetailService.getUserDetailByUid(id);
        return new R<>(OK,data);
    }
